// Pass 3 designer audit screenshots. Real unpacked extension, bundled
// headless Chromium only. Run in small batches via SCENE env var.
// Usage: node dev/designer-pass3.mjs <scene>
...
async function launchExtensionContext() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb-ext-p3-'));
...
const scenes = {
  ...
  // "Almost ready" state, part 1: the grouped/OCR image fixture named in the
  // task, on its own. Note (see PASS-3-designer.md): in this build it saves
  // clean (badge-ok, warn-count=0, no decision rows) via the confirm-yes
  // path - probed directly with dev/zzprobe.mjs before writing this scene,
  // so it is NOT the flagged case; shot anyway per the task's fixture list.
  async home_almost_ready_summit(page) {
    const fixture = path.join(fixturesDir, 'summit_grouped_2line_image.pdf');
    await page.$('#file-input').then((el) => el.setInputFiles(fixture));
    await page.waitForSelector('#attention-cards button:has-text("Set up")', { timeout: 30000 });
    await page.click('#attention-cards button:has-text("Set up")');
    await page.waitForSelector('#confirm-a:not([hidden])', { timeout: 15000 });
    await page.click('#confirm-yes');
    await page.waitForSelector('#confirm-c:not([hidden])', { timeout: 5000 });
    const saveBtn = await page.$('#confirm-c button:has-text("Save")');
    if (saveBtn) await saveBtn.click();
    await page.waitForSelector('#screen-home.active', { timeout: 10000 }).catch(() => {});
    await page.waitForTimeout(500);
    await shot(page, '04a-home-summit-image-saved');
  },

  // "Almost ready" state, part 2: a really flagged file whose Home banner
  // reads "N rows need a quick look" (home.js's decisionRows list) -
  // reliably produced by an auto-matching CSV with duplicate transactions.
  async home_almost_ready(page) {
    const csv = [
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb-p3-csvflag-'));
  ...
  async setup_abc(page) {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb-p3-setup-'));
  ...
  async gear_and_statement_types(page) {
    ...
    await shot(page, '14-statement-types');
  },
  ...
  async settings_and_how(page) {
    ...
    await shot(page, '16-how-it-works');
  },
  ...
  async report_sheet(page) {
    ...
    await page.waitForTimeout(300);
    await shot(page, '17-report-sheet');
  },

  // Review screen with a grouped/OCR PDF: gives a table of extracted rows to
  // check table alignment, centred text, date/number formats against Home.
  async review_screen(page) {
    const fixture = path.join(fixturesDir, 'summit_grouped_2line.pdf');
    await page.$('#file-input').then((el) => el.setInputFiles(fixture));
    ...
    if (checkLink) {
      await checkLink.click();
      await page.waitForSelector('#review-body:not([hidden])', { timeout: 10000 }).catch(() => {});
    }
    await page.waitForTimeout(400);
    await shot(page, '18-review-table');
  },
...
async function runOnboarding() {
  const userDataDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sb-ext-p3-ob-'));
  const context = await chromium.launchPersistentContext(userDataDir, {
    headless: false,
    executablePath: chromium.executablePath(),
    args: [
